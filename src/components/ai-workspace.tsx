...
import { AiDraftLibrary, type AiDraftSaveInput } from "@/components/ai-draft-library";
...
function SmsCopyWorkspace({ account, onSaveDraft }: { account: FlashyAccount; onSaveDraft: (draft: AiDraftSaveInput) => Promise<void> }) {
  ...
  async function saveVariant(index: number) {
    ...
    try {
      await onSaveDraft({ kind: "sms", title: variant.label || objective, content: variant.text, metadata: { objective, audience, offer, mustInclude, basedOnCampaignIds: variant.basedOnCampaignIds } });
      setSavedDraft(index);
    } catch {
      // The parent displays the API error next to the creation tools.
    } finally {
      setSavingDraft(null);
    }
  }
  ...
  return <div>
    ...
    {result ? <div className="pt-5"><div className="mb-4 flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-sm font-black text-[#111318]">טיוטות מוצעות</h3><p className="mt-1 text-[11px] text-[#667085]">{result.model} · נותחו {result.evidence.length} הודעות עבר</p></div><p className="max-w-lg text-xs leading-5 text-[#475467]">{result.patterns.join(" · ")}</p></div><div className="overflow-hidden rounded-lg border border-[#e4e7ec]">{result.variants.map((variant, index) => {
      ...
      return <article key={`${variant.label}-${index}`} className="border-b border-[#eef0f2] p-4 last:border-b-0"><div className="flex items-start justify-between gap-3"><div><h4 className="text-sm font-black">{variant.label}</h4><p className="mt-1 text-[11px] text-[#667085]">{length.characters} תווים · {length.segments} מקטעי SMS משוערים</p></div><div className="flex gap-2"><button type="button" disabled={savingDraft === index || savedDraft === index} onClick={() => void saveVariant(index)} className="inline-flex h-8 items-center gap-1.5 rounded-md bg-[#111318] px-2.5 text-xs font-bold text-white disabled:opacity-55"><BookmarkPlus size={13} />{savingDraft === index ? "שומר..." : savedDraft === index ? "נשמר" : "שמור"}</button><button type="button" onClick={() => copyVariant(variant.text, index)} className="inline-flex h-8 items-center gap-1.5 rounded-md border border-[#d0d5dd] px-2.5 text-xs font-bold"><Copy size={13} />{copied === index ? "הועתק" : "העתקה"}</button></div></div><p className="mt-3 whitespace-pre-wrap rounded-md bg-[#f4fbfa] p-3 text-sm leading-6">{variant.text}</p><p className="mt-3 text-xs leading-5 text-[#475467]">{variant.rationale}</p>{sources.length > 0 && <p className="mt-2 text-[11px] text-[#667085]">מבוסס על: {sources.map((source) => source?.name).join(" · ")}</p>}</article>;
...
function SubjectLineWorkspace({ account, onSaveDraft }: { account: FlashyAccount; onSaveDraft: (draft: AiDraftSaveInput) => Promise<void> }) {
  const [objective, setObjective] = useState("");
  const [audience, setAudience] = useState("");
  const [offer, setOffer] = useState("");
  const [mustInclude, setMustInclude] = useState("");
  ...
  async function saveSubject(pairIndex: number, variantIndex: number) {
    const pair = result?.pairs[pairIndex];
    const variant = variantIndex === 0 ? pair?.variantA : pair?.variantB;
    if (!pair || !variant) return;
    const key = `${pairIndex}-${variantIndex}`;
    setSavingDraft(key);
    try {
      await onSaveDraft({ kind: "subject", title: `${pair.label} · ${variantIndex === 0 ? "A" : "B"}`, content: variant.subject, preheader: variant.preheader, metadata: { objective, audience, offer, mustInclude, tone, hypothesis: pair.hypothesis, basedOnCampaignIds: pair.basedOnCampaignIds } });
      setSavedDraft(key);
    } catch {
      // The parent displays the API error next to the creation tools.
    } finally {
  ...
  return <div>
    ...
    {result ? <div className="pt-5"><div className="mb-4"><h3 className="text-sm font-black">ניסויי A/B מוצעים</h3><p className="mt-1 text-[11px] text-[#667085]">{result.model} · {result.coverage.eligibleCampaigns} מתוך {result.coverage.totalCampaigns} קמפיינים עברו סף של {formatNumber(result.coverage.minimumDelivered)} מסירות</p>{result.patterns.length > 0 && <p className="mt-2 text-xs leading-5 text-[#475467]">{result.patterns.join(" · ")}</p>}</div><div className="overflow-hidden rounded-lg border border-[#e4e7ec]">{result.pairs.map((pair, pairIndex) => {
      ...
      return <article key={`${pair.label}-${pairIndex}`} className="border-b border-[#eef0f2] p-4 last:border-b-0"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="text-sm font-black">{pair.label}</h4><p className="mt-1 text-xs leading-5 text-[#475467]">{pair.hypothesis}</p></div><span className="text-[11px] font-bold text-[#667085]">{confidenceLabels[pair.confidence]}</span></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{([pair.variantA, pair.variantB] as const).map((variant, index) => { const key = `${pairIndex}-${index}`; return <div key={`${variant.subject}-${index}`} className="border-r-2 border-[#20b9a8] bg-[#f7faf9] p-3"><div className="flex items-start justify-between gap-2"><div><p className="text-[10px] font-black text-[#087f72]">וריאציה {index === 0 ? "A" : "B"}</p><p className="mt-1 text-sm font-black leading-6">{variant.subject}</p><p className="mt-1 text-[11px] text-[#667085]">{variant.subject.length} תווים</p></div><div className="flex gap-1.5"><button type="button" aria-label="שמירת שורת נושא" disabled={savingDraft === key || savedDraft === key} onClick={() => void saveSubject(pairIndex, index)} className="grid size-8 shrink-0 place-items-center rounded-md bg-[#111318] text-white disabled:opacity-55"><BookmarkPlus size={13} /></button><button type="button" aria-label="העתקת שורת נושא" onClick={() => copySubject(variant.subject)} className="grid size-8 shrink-0 place-items-center rounded-md border border-[#d0d5dd] bg-white"><Copy size={13} /></button></div></div>{variant.preheader && <p className="mt-3 border-t border-[#dfe7ee] pt-3 text-xs leading-5 text-[#475467]">Preview: {variant.preheader}</p>}{copied === variant.subject && <p className="mt-1 text-[10px] font-bold text-[#087f72]">הועתק</p>}{savedDraft === key && <p className="mt-1 text-[10px] font-bold text-[#087f72]">נשמר בטיוטות</p>}</div>; })}</div>{sources.length > 0 && <div className="mt-3 text-[11px] leading-5 text-[#667085]">מבוסס על: {sources.map((source) => `${source?.name} (${formatPercent(source?.openRate ?? 0)} פתיחה · ${formatCurrency(source?.revenue ?? 0, account.currency)})`).join(" · ")}</div>}</article>;
...
export function AiWorkspace({ clientId, account, summary, emails, sms, automations, plans, onNavigate }: AiWorkspaceProps) {
  ...
  async function saveGeneratedDraft(draft: AiDraftSaveInput) {
    setDraftState("שומר טיוטה...");
    try {
      const response = await fetch("/api/ai/drafts", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ clientId, accountId: account.id, ...draft }) });
      const payload = await response.json();
      if (!response.ok || !payload.success) throw new Error(payload.message || "שמירת הטיוטה נכשלה.");
      setDraftRefreshKey((current) => current + 1);
      setDraftState("הטיוטה נשמרה.");
    } catch (error) {
