...
export function ClientOnboardingWizard() {
  ...
  async function analyzeDocuments() {
    ...
    try {
      ...
      setAiModel(payload.model ?? "OpenAI");
      setMessage(`נבנה פרופיל ראשוני באמצעות ${payload.model || "OpenAI"}. אפשר לעבור על השאלות לפני יצירת הלקוח.`);
    } catch (error) {
  ...
  async function createClient() {
    ...
    try {
      const response = await fetch("/api/live-client", {
        ...
        body: JSON.stringify({ apiKey, clientName, industry, smsCreditPriceUsd, monthlySubscriptionCostUsd, agencyRetainerCostIls, usdIlsRate, visibleModules, documents: material, onboarding, clientUserName, clientEmail }),
      });
      const payload = await response.json();
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-[#e4e7ec] bg-white shadow-[0_8px_24px_rgba(16,24,40,0.04)]">
      ...
      <div className="mx-auto max-w-3xl px-5 py-5 sm:px-6">
        ...
        {step === 4 && <div><div className="rounded-lg bg-[#f8fafc] p-4 text-sm leading-6 text-[#667085]">אפשר לדלג וליצור משתמש מאוחר יותר. אם מוסיפים אימייל עכשיו, הלקוח יוכל לבקש אליו קוד כניסה חד־פעמי. אין צורך ליצור או להעביר סיסמה.</div><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="text-sm font-bold text-[#344054]">שם משתמש<input className={fieldClass} value={clientUserName} onChange={(event) => setClientUserName(event.target.value)} /></label><label className="text-sm font-bold text-[#344054]">אימייל מורשה<input type="email" dir="ltr" className={`${fieldClass} text-left`} value={clientEmail} onChange={(event) => setClientEmail(event.target.value)} /></label></div></div>}
        {step === 5 && <div className="divide-y divide-[#eaecf0] rounded-xl border border-[#e4e7ec]">{[["לקוח",clientName],["Flashy",`${flashyAccount?.name || flashyAccount?.account} · ${flashyAccount?.id}`],["מודולים",visibleModules.join(" · ")],["מסמכים",`${documents.length + (manualContext.trim() ? 1 : 0) + (onboardingAnswers.trim() ? 1 : 0)} מסמכים/מקורות`],["פרופיל AI",onboarding ? `נסרק באמצעות ${aiModel || "OpenAI"}` : "לא נסרק"],["משתמש",clientEmail || "ייווצר בהמשך"]].map(([label,value]) => <div key={label} className="grid grid-cols-[120px_1fr] gap-4 p-4 text-sm"><span className="font-bold text-[#667085]">{label}</span><span className="font-bold text-[#111318]">{value}</span></div>)}</div>}
      </div>
